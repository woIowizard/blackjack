...
The scripts in this repo help to formulate a card counting strategy for Blackjack.<br>
...
It is known that Blackjack can be beaten with card counting. It is also thought that continuous shuffling machines (CSMs) impede card counting by making forthcoming cards unpredictable. In practice, CSMs have a mechanical feature that allows for some (limited) degree of card counting. Using this pair of scripts, it can be shown that the allowed extent of card counting is exploitable to give the player a positive edge over the house. Blackjack.py simulates Blackjack games at a rate of about 6 million rounds per hour, which allows the long term results of strategies to be tested. Stratfind.py uses an approximate brute-force algorithm to find a betting strategy that gives the player a positive edge. A strategy is found with a 0.04% theoretical edge in favour of the player, which materialised as a 0.09% profit in an experiment of 10 million rounds.

<hr />

## blackjack.py

In its default setting, blackjack.py simulates a single round of Blackjack according to the rules [here](https://www.gra.gov.sg/docs/default-source/game-rules/mbs/blackjack-pontoon-games/blackjack-pontoon---gra-website/mbs-blackjack-game-rules-version-63c8994ee-f3e8-4bfd-8398-dee76aa466ee.pdf?sfvrsn=3d3481e5_1). The simulated player opens 5 pockets with bets of $25 on each, and follows basic strategy. To see the simulated player's decisions in action, run `blackjack -V`.
...
To run a simulation of 5 million rounds under basic strategy, run `blackjack -n 5M`

![standard-edge](img/standard-edge.png)

The results of the simulation come close to [known values](https://wizardofodds.com/games/blackjack/calculator/). This verifies that basic strategy alone is insufficient to give the player a favourable edge.

![known-baseline](img/known-baseline.png)

We aim to formulate a strategy that gives the player a favourable edge.

<hr />

## Card-counting theory

A popular method for beating Blackjack is _card counting_, in which a player uses their knowledge of dealt cards to infer whether forthcoming cards are likely to be in their favour. The count at any point is the number of high cards (10 to A) that have been dealt subtracted from the number of low cards (2 to 6) that have been dealt. A high (low) count indicates that forthcoming cards are likely to be high (low), which favours the player (house). A betting strategy in which the player bets conservatively at low counts and aggressively at high counts may give the player a positive overall edge.

Card-counting works best when cards played are not shuffled back into the deck, but continuous shuffling machines (CSMs) allow the dealer to shuffle, at the end of each round, the cards dealt in that round back into the deck. In theory, this prevents card counting, because it implies that the cards dealt in each round is independent of those dealt in previous rounds. But in practice, CSMs maintain a small _buffer_ of cards that are not shuffled when cards are fed into the CSM, to smoothen dealing. This makes the count in a given round indicative of the likelihood of high cards coming from that buffer. It turns out that this degree of card counting, albeit extremely limited, admits a strategy that gives the player a favourable edge.

In its default setting, blackjack.py implements a CSM containing 6 decks and with a 7-card buffer, in line with the apparent specifications of the CSM used in the casino at Marina Bay Sands. These parameters can be overridden with the -D and -B options.

<hr />

## Strategy formulation

Formulating a strategy requires knowledge of how the house edge varies with the count. For CSMs with a small buffer, the house edge also depends on the number of pockets played (since a larger number of pockets runs out the buffer more quickly, increasing the proportion of cards in a round not indicated by the count). Using blackjack.py, we ran simulations of 5 million rounds at each number of pockets from 1-5, and each count value from -10 to +15. The per-count estimated house edges are as follows:
...
Unlike the legacy shuffling system, in which the house edge at a count is independent of the number of pockets played, some counts under a CSM admit player-favourable edges only at low pocket numbers. This was anticipated above, and must be considered when deciding how aggressive a betting strategy can be in terms of number of pockets. 

The number of pockets played also indirectly affects the house edge in the next round, because playing a larger number of pockets in one round increases the probability that a more extreme-valued count would obtain in the next round. To measure this effect, we need to know the _transition probabilities_ from number of pockets to counts. For each number of pockets from 1-5, we want to estimate the probability that a round played with that number of pockets will end in a particular count. We assume for simplicity that the cardplay strategy does not change with count, so that the starting count value is unimportant. The following command runs a simulation of 10 million rounds with 1 pocket each, reporting the proportion of rounds that end in each count from -10 to +15: `blackjack -p 1 -n 10M -T`.
...
Stratfind.py uses this data to formulate a favourable betting strategy. The script takes as input two matrices containing the per-count edges and transition probabilities, and runs an approximate brute-force algorithm to find a betting strategy that gives the player an edge above a specified threshold. The strategy to be identified has the following form<br>
...
This betting strategy follows the pattern of usual card-counting strategies: conservative at unfavourable counts and aggressive at favourable counts. 

To improve the robustness of our results against random error in the trials above, the edge values are adjusted by .0015 in favour of the house, which forces stratfind to find a strategy that favours the player even under possible handicap. In its default setting, stratfind takes the minimum bet to be $25, searches upward from x=1 without limit, and seeks a strategy with positive edge. These can be overridden with command-line options

![stratfind-help](img/stratfind-help.png)

Given the matrices above, a full brute-force search would be impractical (5^12 > 200M trials for each value of x). Stratfind in its default setting does only an approximate brute-force search in which the number of pockets at odd-valued counts are assumed to be identical to those at the immediately-lower count--this can be overridden with the -F option. The approximation reduces the search space to 15625 trials at each value of x, which can be performed in below 5 seconds. Running `stratfind` without options begins the search:

![stratfind-first](img/stratfind-first.png)

Stratfind finds that a positive edge of 0.0116% is possible at x=2. To find a more robust edge, we ran a brute-force search beginning at x=3 and set the threshold to 0.02%: `stratfind -s 3 -b .0002`.

![stratfind-second](img/stratfind-second.png)

A strategy of edge 0.0444% is possible at x=3. Given the uniformity of pocket numbers from counts -10 to +1, a full brute force search looks unnecessary. Our strategy is now:
...
## Validation
Running blackjack.py with `-S` implements this strategy. As validation, we ran a simulation of 10 million rounds: `blackjack -n 10M -S`.

![betstrat-confirm](img/validation.png)

Observations:
- The player came out with a .09% profit.
- The profit margin was positive (albeit with some fluctation) at every intermediate point taken at 1 million round intervals, indicating that this result is stable.
- There were significantly more losing rounds than profitable rounds, with 42% of rounds being profitable and 50% of rounds losing.
- The profitable rounds at favourable counts (>= +2) returned gains that outweighed the losses from losing rounds, which led to the overall favourable edge.
The last two features are typical of card counting strategies. So, card counting can beat Blackjack even in the presence of a continuous shuffling machine. 
...
Presumably, basic strategy is the optimal count-insensitive cardplay strategy. With counting in view, it might be advantageous to deviate from basic strategy at certain count ranges. We will test the possible profitability of the following deviations from basic strategy:
...
The other possible deviations did not show significant improvements from basic strategy at either low, high, or middling counts. More thorough tests of the shortlisted viable deviations were run with simulations of 3 million rounds at each count from -10 to 15. The per-count edges for each viable deviation are as follows:
...
The results showed significant variance due to the relatively small number of rounds, but they look sufficient to conclude that deviating from basic strategy would not yield significant edge improvements even with counting.


<hr />

## How practical is this?
The results above indicate theoretical long-term profits. In practice, time and money constraints might limit the effectiveness of card-counting as a money-making method. To see how the above strategy might play out in practice, we ran 144000-round simulations (approximating the number of rounds played in a year: 50 weeks x 48 hours/week x 1 round/min). The following command in Powershell runs 100 such simulations: `foreach ($i in 1..100){echo "===== TRIAL $i =====";blackjack -S -n 144000;echo "";echo ""}`. The raw results from these trials are in practical-trial.txt. In summary:
...
Although the average estimated annual profit is positive, the negative loss can go below -80k, and indeed goes below -50k in 14% of trials. The average annual profits and highest positive profits also varied quite widely. So the strategy above, despite being theoretically profitable, rather risky as a money-making method in practice.
